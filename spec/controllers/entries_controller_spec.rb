...
describe EntriesController do
  ...
  context 'POST create' do
    context 'with valid parameters' do
      let(:valid_attributes) {{:word => 'tree', :definition => 'tall leafy bush'}}
  ...
  context 'POST create' do
   ...
   context 'GET show' do
    ...
    context 'PUT update' do 
      ...
      context 'with valid parameters' do
      let(:valid_attributes) {{:word => 'orange', :definition => 'large bush fruit'}}
      let(:valid_parameters) {{:id => entry.id, :entry => valid_attributes}}
      before {put :update, valid_parameters}

      it 'updates the entry' do 
        Entry.find(entry.id).word.should eq valid_attributes[:word]
  ...
  context 'GET index' do
    before {Entry.create({:word => 'peach', :definition => 'juicy and delicious'})}
    before {get :index}

    it {should respond_with 200}
    it {should respond_with_content_type :json}
    it 'responds with a json representation of all the entries' do
